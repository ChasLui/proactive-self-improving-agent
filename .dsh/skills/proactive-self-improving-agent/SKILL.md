...
description: "自改进技能：捕获并沉淀可复用经验。触发：(1)命令/操作失败 (2)被用户纠正('不对'/'应该是'/'Actually') (3)用户需要不存在的能力 (4)外部API/工具出错 (5)知识过时或错误 (6)发现更好做法 (7)任务完成回顾。写入 .learnings/ 三文件体系(LEARNINGS/ERRORS/FEATURE_REQUESTS)并追加 JSONL 日志；先判断是否新颖，已有条目已覆盖则跳过；同一模式≥3次自动晋升，满足条件可提取为独立 skill。详见正文。"
whenToUse: "检测到 7 种触发场景且存在真正新颖、未被已有条目覆盖的经验时使用；任务完成时回顾本轮有无新经验；不确定时先读正文『6. 快速参考』。"
version: 1.1.0
