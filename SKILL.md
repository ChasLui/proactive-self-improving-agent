...
description: 自动捕获经验并安全进化。命令/工具/API 失败、被用户纠正（"不对"/"应该是"/"Actually"）、发现自己知识过时或错误、发现更好做法、用户需要不存在的能力、任务完成回顾——先评估是否有可复用的新经验，有则写入 ~/.claude/.learnings/，反复出现或高价值时晋升到 memory 沉淀层 / CLAUDE.md / AGENT-SKILLS.md / 独立 skill。写入前先去重：memory 沉淀层或已有条目已覆盖则跳过。详见正文。
---
...
首次使用时若目录不存在，从本 skill 的 `templates/learnings/` 拷贝初始化（在 skill 仓库根目录执行；reasonix 触发时也会自动完成）：

```bash
mkdir -p ~/.claude/.learnings && cp -n templates/learnings/*.md ~/.claude/.learnings/
```

> 若某条经验**只对当前 repo 有意义**（该项目特有的构建命令、目录约定），写 `<repo>/.learnings/` 而不是用户级。判据：换一个项目还会用到吗？

---

## 3. 写入前置检查（顺序不可换）

1. **查已晋升的沉淀层**：`grep -ri "<关键词>" ~/.claude/projects/*/memory/ 2>/dev/null`（或用 memory 工具搜索）
   - 命中且已覆盖 → **完全跳过**，什么都不写。
2. **查经验池**：`grep -ri "<关键词>" ~/.claude/.learnings/`
   - 命中相似条目 → 不新建，给旧条目加 `See Also` 互链，复用同一 `Pattern-Key`。
3. 都没命中 → 新建条目 + 追加 CHANGELOG。

> 沉淀层里已有的东西，经验池不再重复收录。这是两层不互相污染的唯一保证。
...
`.learnings/` 是原始池（容忍噪音），memory 沉淀层是永久文件（零噪音）。**单向晋升，禁止同一经验两轨同写。**

| 经验类型 | 晋升到 | 说明 |
|---|---|---|
| 跨项目的行为准则（可验证、能防错、复用概率高） | `~/.claude/projects/*/memory/`（memory 沉淀层，`feedback_*.md`） | 常驻检索，用 memory 工具或 grep 可达 |
| 更高层级的全局规则 | `~/.claude/CLAUDE.md` / `AGENT-SKILLS.md` | 每次会话常驻 context |
| 本项目的约定 | `./CLAUDE.md` / `./AGENTS.md` | 随仓库 commit，团队共享 |
| 需要多步操作的流程 | `~/.claude/skills/<name>/SKILL.md` 或 `<repo>/.reasonix/skills/` | 按需加载，不占常驻 context |

**一条规则 → 沉淀层 / CLAUDE.md；一套流程 → skill。** 这是选择依据。
...
晋升必须同时满足：**可验证、能预防重复错误、未来复用概率高、不会增加不必要复杂度**。
...
写入沉淀层时带 frontmatter，正文用 Why / How to apply 结构：

```markdown
---
name: <一句话主题>
description: <检索用的一句话描述>
type: feedback
---
## 反 pattern / 经验
...
...
- 晋升会改 `CLAUDE.md` / memory 沉淀层这类**长期生效**的文件 —— **改前告诉用户改了什么**，不静默进化。
...
```text
触发 → grep memory 沉淀层 ── 已覆盖 ──→ 跳过（什么都不写）
        │未覆盖
        ▼
      ...
      .learnings/ 新条目 + CHANGELOG add
        ...
        ▼
      memory 沉淀层 / CLAUDE.md + CHANGELOG promote（并告知用户）
        │
        │ 是流程而非规则
        ▼
      ~/.claude/skills/<name>/ 或 .reasonix/skills/<name>/ + CHANGELOG extract
```

### 写入检查清单

- [ ] 查过 memory 沉淀层？已覆盖则跳过
